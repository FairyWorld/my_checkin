...
function server(msg) {
  return new Promise(async (resolve) => {
    try {
      ...
      let data = `title=${encodeURI("my_checkin每日任务已完成")}&desp=${encodeURI(
        msg.replace(/\n/g, "\n\n")
      )}`;
...
function tgpush(msg) {
  return new Promise(async (resolve) => {
    try {
      ...
      let url = `https://telegram_proxy.lulafun.workers.dev/bot${tgbotoken}/sendMessage?parse_mode=Markdown&text=${encodeURI(
        msg.replace(/【|】/g, "*")
      )}&chat_id=${chatid}`;
      //  let res = await axios.post(url,data);
